...
The **ax_task_delete** was also inserted in **task_runtime** to prevent calling the **sched_switch** routine in user mode.

# Consequences

By adding **_task_start** and **ax_task_delete**, the system ensures that tasks can only run in **user mode** and access to privilege ressources through the **syscall** api.